...
// 入口主函数 
// main函数会有异步任务 async + await
async function main() {
    // 和AI LLM  chat 从编程思维-> AI编程思维 -> AIGC -> AGI
    // client 是 openai 对象 chat 聊天 完成任务.
    // LLM 是在远程运行 await
    // 字符串模板 提示词
    const prompt = `
    <tr class="item">
        ...
        这是一段电影html代码,请获取电影名(name),封面链接(picture),简介(info),评分(score),
        评论人数(commentNumber),请使用括号的单词作为属性名,以JSON的格式返回。
    `
